...
import net.mcreator.outerendwilds.mixins.NoiseGeneratorSettingsAccess;

@EventBusSubscriber
public class OuterEndWildsModSurfaceRules {
	@SubscribeEvent
	public static void init(ServerAboutToStartEvent event) {
		...
		boolean hasEndBiomes = chunkGenerator.getBiomeSource().possibleBiomes().stream().anyMatch(biomeHolder -> biomeHolder.unwrapKey().orElseThrow().location().getNamespace().equals("outer_end_wilds"));
		if (hasEndBiomes) {
			if (chunkGenerator instanceof NoiseBasedChunkGenerator generator) {
				...
				registerSurfaceRules(ResourceLocation.parse("outer_end_wilds:end_wilds"), noiseGeneratorSettings, OuterEndWildsModBlocks.WILD_NYLIUM.get().defaultBlockState(), Blocks.END_STONE.defaultBlockState());
			}
		}
	}
